    ...
    public static void result(String s1, String s2) {
        ...
        for (int i = 1; i <= s1.length(); i++) {
            for (int j = 1; j <= s2.length(); j++) {
                if (s1.charAt(i-1) == s2.charAt(j-1)) {
                    ...
                    strings[i][j] = strings[i-1][j-1].length() > strings[i][j-1].length() ?
                            new StringBuilder(strings[i-1][j-1] + s) : new StringBuilder(strings[i][j-1] + s);
                }
                else {
        ...
        for (int i = 1; i <= s1.length(); i++) {
            for (int j = 1; j <= s2.length(); j++) {
                if (s1.charAt(i-1) == s2.charAt(j-1)) {
                    String s = String.valueOf(s1.charAt(i - 1));
                    strings2[i][j] = strings2[i-1][j-1].length() >= strings2[i][j-1].length() ?
                            new StringBuilder(strings2[i-1][j-1] + s) : new StringBuilder(strings2[i][j-1] + s);
                }
                else {
